...
echo "Enter the email adress of your first account:"
read email1
echo "Enter the name of your first account:"
read name1

echo "Enter the email adress of your second account:"
read email2
...
eval "$(ssh-keygen -t rsa -b 4096 -C email1 -f name1)"
eval "$(ssh-keygen -t rsa -b 4096 -C email2 -f name2)"
echo "\n\nssh keys created successfully."

echo "Copying $name1 ssh keys to clipboard...\n"
eval "$(pbcopy < ~/.ssh/$name1.pub)"
...
read response
if [ response == "1" ]; then
    echo "Using $name1 with $email1 by default."
    git config --global user.name $name1
    git config --global user.email $email1
elif [ response == "2" ]; then
    echo "Using $name2 with $email2 by default."
